...
export const Step5Validate: React.FC<{job:AudiobookJob;onValidate:()=>Promise<void>;isValidating:boolean}> = ({job,onValidate,isValidating}) => <div className="bg-white border rounded-xl p-6 space-y-4">
  ...
  <button className="bg-amber-600 text-white rounded px-4 py-2 disabled:opacity-40" title="Read the exported file and compare its chapters with the saved list" disabled={isValidating || !job.outputM4b} onClick={onValidate}>{isValidating ? 'Checking file…' : 'Check exported file'}</button>
