...
test('HTTP transcription rerun retains cache; beam invalidates; lead-in and model-panel preferences reuse words', { timeout: 60000 }, async () => {
  ...
  const server = spawn(process.execPath, ['--require', trace, entry], { env: { ...process.env, APP_ROOT: root, PORT: String(port), NODE_ENV: 'production' }, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
  let logs = '';
