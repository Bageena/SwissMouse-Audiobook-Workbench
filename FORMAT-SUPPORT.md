...
3. Compare original chapters with the editable chapter list. Save the reviewed list; only its start/end ranges are exported. Include an opening marker at `00:00:00.000`, since MP4 chapter tracks require a marker at zero. By default, ranges cover the recording through its end. Explicit gaps and a shortened final end are now removed from exported audio, with output chapters and CUE positions rebased. The automatic 1 ms metadata separator is not an audio cut. Overlaps are rejected. Source audio and editor timestamps remain unchanged.
4. Edit metadata/artwork, then select any combination of M4B, M4A, MP3, FLAC, Ogg Vorbis, Opus and WAV. The source format is selected by default when offered; other inputs default to M4B. The interface states copy or conversion per output. Precise range cuts require re-encoding; continuous ranges retain the existing copy path unless conversion is selected or the codec/quality is incompatible.
...
### Chapter editor and theme regression (September 2026)

The former exporter wrote chapter start/end metadata around the complete source; gaps were not physically removed. The current exporter trims reviewed ranges, concatenates them once, and rebases embedded chapters, WAV markers, and CUE positions. Overlaps still fail validation rather than duplicating audio. Tests cover all seven output formats, exact retained PCM samples for WAV/FLAC, source-file fingerprints, default uncut stream-copy behavior, and final HTTP validation of a shortened recording.

Across the current regression files, 132 tests passed and three optional integrations skipped (external Audiobookshelf parser and two real speech-recognition checks). No models were downloaded. TypeScript, production build, and whitespace checks passed.

An isolated 180-second browser fixture verified Numeric/Roman/Written updates, manual-title protection, unchanged timestamps when changing formats, session persistence, start/end selection, repeated seeks, waveform keyboard seeking, interactive word clicks, focus retention, and live playback highlighting. Six built-in themes, the example custom theme, Reload Themes, appearance persistence, dark-theme notice contrast, and terminal colors were checked. Indexed transcript lookup and bounded player word windows avoid rescanning the complete transcript on playback ticks. These checks do not certify every external audiobook player or multi-hour export performance.

For repeatable manual editor checks on Windows, run `npm run build`, then `node tools/preview-editor.mjs` and open `http://127.0.0.1:3108`. The fixture uses the managed `runtime/bin/ffmpeg.exe`, synthetic audio, and an isolated in-memory project; it does not read or modify production jobs.
